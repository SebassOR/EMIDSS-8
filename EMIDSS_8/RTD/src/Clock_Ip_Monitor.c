...
#define CLOCK_IP_MONITOR_SW_MAJOR_VERSION_C               7
...
#define CLOCK_IP_MINIMUM_REGISTER_VALUE           3U
#define CLOCK_IP_MAXIMUM_REGISTER_VALUE    0xFFFFFCU
    ...
    if ((NULL_PTR != Config) && (Config->Enable != 0U))
    {
        ...
        if (HashCmu[Index] != ((((uint32)Config->MonitoredClockFrequency)  ^ ((uint32)Config->Name))))
        {
            ...
            MonitoredClk = Config->MonitoredClockFrequency;
                ...
                CLOCK_IP_DEV_ASSERT((ReferenceClk != 0U) && (ReferenceClk < 65536000U) && (BusClk != 0U) && (MonitoredClk != 0U) );
            #endif
            ...
            if ((ReferenceClk != 0U) && (BusClk != 0U) && (MonitoredClk != 0U))
            {
                ...
                ((Clock_Ip_ClockMonitorType*)CmuInformation->CmuInstance)->HTCR = ((HfRef) > CLOCK_IP_MAXIMUM_REGISTER_VALUE)? CLOCK_IP_MAXIMUM_REGISTER_VALUE : ((uint32)HfRef);

                /* Set low limit. Do not program LFREF to a value less than 0x00000003 */
                ((Clock_Ip_ClockMonitorType*)CmuInformation->CmuInstance)->LTCR = ((LfRef) < CLOCK_IP_MINIMUM_REGISTER_VALUE)? CLOCK_IP_MINIMUM_REGISTER_VALUE : ((uint32)LfRef);
            }
        }
