...
#define LPI2C_IP_AR_RELEASE_MINOR_VERSION_C       9
#define LPI2C_IP_AR_RELEASE_REVISION_VERSION_C    0
#define LPI2C_IP_SW_MAJOR_VERSION_C               7
 ...
 * Local Functions
 ******************************************************************************/
 static void Lpi2c_Ip_MasterClearStatus_INTR(LPI2C_Type * const BaseAddr);
 static void Lpi2c_Ip_SlaveClearStatus_INTR(LPI2C_Type * const BaseAddr);
 #if (STD_ON == LPI2C_IP_DMA_FEATURE_AVAILABLE)
...
#if (STD_ON == LPI2C_IP_DEV_ERROR_DETECT)
        /* Hardware FIFO full, use software FIFO */
        DevAssert(Master->CmdQueue.ReadIdx < LPI2C_MASTER_CMD_QUEUE_SIZE);
#endif
                                       ...
                                       boolean Receive)
{
...
#if ((STD_ON == LPI2C_IP_HAS_ULTRA_FAST_MODE) && (STD_ON == LPI2C_IP_ERRATA_ERR052121))
    ...
    else
    {
        ...
        AddrByte = (uint8)(((Master->SlaveAddress << 1U) + (uint8)((Receive == TRUE) ? 1U : 0U)) & 0xFFU);
        Lpi2c_Ip_MasterQueueCmd(BaseAddr, Master, StartCommand, AddrByte);
    }
}
 ...
 *
 *END**************************************************************************/
static void Lpi2c_Ip_SlaveConfigureNormalMode(LPI2C_Type *BaseAddr, uint8 ClockStretching)
{
    ...
    LPI2C_Set_SlaveClkHold(BaseAddr, ClockStretching);
}
...
#endif
    {
        Lpi2c_Ip_SlaveConfigureNormalMode(BaseAddr, ClockStretching);
    }
                                                       ...
                                                       Lpi2c_Ip_MasterStateType *Master)
{
    ...
    else
    {
        ...
        if (((Master->ByteReceiveCount + 4U) % I2C_MAX_DATA_RECEIVE_BYTE_PER_CMD) == 0U)
        {
            ...
            {
                /* Queue receive command for RxSize bytes */
                Lpi2c_Ip_MasterQueueCmd(BaseAddr, Master, LPI2C_MASTER_COMMAND_RECEIVE, (uint8)((Master->BufferSize - 5U) & 0xFFU));
            }
        }
    }
}
...
#if defined(LPI2C_IP_ERRATA_E10792)
                                    ...
                                    const Lpi2c_Ip_BaudRateType * BaudRate)
{
    ...
    /* Set hold delay */
    LPI2C_Set_MasterSetupHoldDelay(BaseAddr, (uint8)(BaudRate->SetHold & 0xFFU));
    /* Set data valid delay */
    LPI2C_Set_MasterDataValidDelay(BaseAddr, (uint8)(BaudRate->DataValid & 0xFFU));
    /* Set clock HIGH period */
    LPI2C_Set_MasterClockHighPeriod(BaseAddr, (uint8)(BaudRate->ClkHI & 0xFFU));
    /* Set clock low period */
    LPI2C_Set_MasterClockLowPeriod(BaseAddr, (uint8)(BaudRate->ClkLO & 0xFFU));
}
                                                ...
                                                uint32 InputClock)
{
    ...
    /* Check to be able to do Baudrate << 7U */
    DevAssert(Baudrate <= 0xFE000000U);
    /* Avoid possible overflow for ClkTotal calculation */
    DevAssert((0xFFFFFFFFU - InputClock) > (Baudrate << 6U));
#endif
                         ...
                         const Lpi2c_Ip_MasterConfigType * ConfigPtr)
{
    ...
    Master->DebugMode = ConfigPtr->DebugMode;
    ...
    if (TRUE == ConfigPtr->DebugMode)
    {
        /* Enable Debug Mode for Master */
        LPI2C_Set_MasterEnableDebugMode(BaseAddr, Master->DebugMode);
    }
...
void Lpi2c_Ip_MasterDeinit(uint8 Instance)
{
    ...
    if (TRUE == Master->DebugMode)
    {
        /* Disable Debug Mode for Master */
        LPI2C_Set_MasterEnableDebugMode(BaseAddr, FALSE);
    }
                                                    ...
                                                    uint32 Timeout)
{
    ...
    if (Master->I2cIdle)
    {
        ...
        do
        {
            ElapsedTicks += IterationTicks;
            /* Master send data */
            Lpi2c_Ip_MasterSend(Instance);
            IterationTicks = OsIf_GetElapsed(&CurrentTicks, I2C_TIMEOUT_TYPE);
            RetStatus = Lpi2c_Ip_MasterGetTransferStatus(Instance, NULL_PTR);
        } while ((RetStatus == LPI2C_IP_BUSY_STATUS) &&
                 (IterationTicks < (TimeoutTicks - ElapsedTicks)));

        /* Check Timeout */
        if (LPI2C_IP_BUSY_STATUS == RetStatus)
        {
                                                ...
                                                uint32 RxSize)
{
    ...
    {
        /* Queue receive command for RxSize bytes */
        Lpi2c_Ip_MasterQueueCmd(BaseAddr, Master, LPI2C_MASTER_COMMAND_RECEIVE, (uint8)((RxSize - 1U) & 0xFFU));
    ...
    /* Set RX watermark */
    LPI2C_Set_MasterRxFIFOWatermark(BaseAddr, (uint16)((RxBytes - 1U) & 0xFFFFU));
}
                        ...
                        const Lpi2c_Ip_SlaveConfigType * ConfigPtr)
{
    ...
    Lpi2c_Ip_SlaveSetOperatingMode(Instance, ConfigPtr->OperatingMode, ConfigPtr->ClockStretching);
...
static void Lpi2c_Ip_SlaveIRQHandlerInternal(uint8 Instance)
{
    ...
    {
        if ((1U == Slave->IsFirstRepeatedStart) && (Slave->Is10bitAddress))
        {
            RepeatStartDetect = FALSE;
            LPI2C_Clear_SlaveRepeatedStartEvent(BaseAddr);
        }
        Slave->IsFirstRepeatedStart = 0U;
    }
...
void Lpi2c_Ip_MasterIRQHandler(uint8 Instance)
{
    ...
    {
        /* CPR_RTD_00012
         * The driver is not initialized, clear interrupt status flag only for the enabled interrupts and return immediately
         */
        Lpi2c_Ip_MasterClearStatus_INTR(Lpi2c_Ip_pxBase[Instance]);
    }
    else
...
void Lpi2c_Ip_SlaveIRQHandler(uint8 Instance)
{
    ...
    if (NULL_PTR == Lpi2c_Ip_pxSlaveState[Instance])
    {
         ...
         * The driver is not initialized, clear interrupt status flag only for the enabled interrupts and return immediately
         */
        Lpi2c_Ip_SlaveClearStatus_INTR(Lpi2c_Ip_pxBase[Instance]);
    }
    else
#endif /* LPI2C_IP_COMMON_IRQ_MASTER_AND_SLAVE */
    {
...
void Lpi2c_Ip_ModuleIRQHandler(uint8 Instance)
{
    ...
    {
        /* CPR_RTD_00012: The driver is not initialized, clear interrupt status flag only for the enabled interrupts and return immediately */
        BaseAddr = Lpi2c_Ip_pxBase[Instance];
        Lpi2c_Ip_MasterClearStatus_INTR(BaseAddr);
        Lpi2c_Ip_SlaveClearStatus_INTR(BaseAddr);
    }
    else
    {
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Lpi2c_Ip_MasterClearStatus_INTR
 * Description   : Clear Lpi2c master status flag only for enabled interrupts.
 *
 *END**************************************************************************/
static void Lpi2c_Ip_MasterClearStatus_INTR(LPI2C_Type * const BaseAddr){

    uint32 flags = BaseAddr->MIER & (LPI2C_MIER_DMIE_MASK | LPI2C_MIER_PLTIE_MASK | LPI2C_MIER_FEIE_MASK | LPI2C_MIER_ALIE_MASK |
                                       LPI2C_MIER_NDIE_MASK | LPI2C_MIER_SDIE_MASK  | LPI2C_MIER_EPIE_MASK);
    /* RDF and TDF flags in LPI2C_MSR are cleared automatically by hardware upon FIFO access */

    LPI2C_Clear_MasterStatusFlags(BaseAddr,flags);
}

/*FUNCTION**********************************************************************
 *
 * Function Name : Lpi2c_Ip_SlaveClearStatus_INTR
 * Description   : Clear Lpi2c slave status flag only for enabled interrupts.
 *
 *END**************************************************************************/
static void Lpi2c_Ip_SlaveClearStatus_INTR(LPI2C_Type * const BaseAddr){
    uint32 SIER = BaseAddr->SIER;

    uint32 flags = SIER & (LPI2C_SIER_SARIE_MASK | LPI2C_SIER_GCIE_MASK | LPI2C_SIER_AM1IE_MASK | LPI2C_SIER_AM0IE_MASK |
    ...
    flags = SIER & (LPI2C_SIER_FEIE_MASK | LPI2C_SIER_BEIE_MASK | LPI2C_SIER_SDIE_MASK | LPI2C_SIER_RSIE_MASK);
    LPI2C_Clear_SlaveStatusFlags(BaseAddr,flags);

    if((SIER & LPI2C_SIER_TAIE_MASK) != 0u)
    {
        (void)(BaseAddr->STAR);
    }

    if((SIER & LPI2C_SIER_RDIE_MASK) != 0u)
    {
        (void)(BaseAddr->SRDR);
    }

    if((SIER & LPI2C_SIER_TDIE_MASK) != 0u)
    {
        (void)(BaseAddr->STDR);
    }
}
