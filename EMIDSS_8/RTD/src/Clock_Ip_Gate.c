...
#define CLOCK_IP_GATE_SW_MAJOR_VERSION_C               7
...
static void Clock_Ip_ClockEnableGate(uint32 Partition, uint32 Collection, uint32 EnableRequest, Clock_Ip_NameType ClockName);
static void Clock_Ip_ClockDisableGate(uint32 Partition, uint32 Collection, uint32 EnableRequest, Clock_Ip_NameType ClockName);
    ...
    #endif
    {
...

        {
            ...
            if (TRUE == TimeoutOccurred)
            {
                ...
                Clock_Ip_ReportClockErrors(CLOCK_IP_REPORT_TIMEOUT_ERROR, ClockName);
            }
        }
    }
}
...
static void Clock_Ip_ClockSetGateMcMePartitionCollectionClockRequest(Clock_Ip_GateConfigType const* Config)
{
    ...
    if (NULL_PTR != Config)
    {
        ...
        if (Config->Enable != 0U)
        {
            /*progress Enable clock gate on MC_ME*/
            Clock_Ip_ClockEnableGate(Partition,Collection,EnableRequest,Config->Name);
        }
        else
        {
            /*progress Disable clock gate on MC_ME*/
            Clock_Ip_ClockDisableGate(Partition,Collection,EnableRequest,Config->Name);
        }
    }
    else
    {
