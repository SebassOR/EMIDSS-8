...
#define DET_AR_RELEASE_MINOR_VERSION_C      9
#define DET_AR_RELEASE_REVISION_VERSION_C   0
#define DET_SW_MAJOR_VERSION_C              7
...
uint8 Det_InstanceId[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];               /**< @brief DET instance ID*/
uint8 Det_ApiId[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];                    /**< @brief DET API ID*/
uint8 Det_ErrorId[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];                  /**< @brief DET Error ID*/
/* Variables to store last DET runtime error */
uint8 Det_RuntimeInstanceId[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];        /**< @brief DET Runtime instance ID*/
uint8 Det_RuntimeApiId[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];             /**< @brief DET Runtime API ID*/
uint8 Det_RuntimeErrorId[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];           /**< @brief DET Runtime Error ID*/
/* Variables to store last DET transient error */
uint8 Det_TransientInstanceId[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];      /**< @brief DET Transient instance ID*/
uint8 Det_TransientApiId[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];           /**< @brief DET Transient API ID*/
uint8 Det_TransientFaultId[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];         /**< @brief DET Transient Error ID*/
#define DET_STOP_SEC_VAR_CLEARED_8_NO_CACHEABLE
#include "Det_MemMap.h"

#define DET_START_SEC_VAR_CLEARED_16_NO_CACHEABLE
#include "Det_MemMap.h"
uint16 Det_TransientModuleId[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];       /**< @brief DET Transient module ID*/
uint16 Det_ModuleId[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];                /**< @brief DET module ID*/
uint16 Det_RuntimeModuleId[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];         /**< @brief DET Runtime module ID*/
#define DET_STOP_SEC_VAR_CLEARED_16_NO_CACHEABLE
#include "Det_MemMap.h"

#define DET_START_SEC_VAR_CLEARED_32_NO_CACHEABLE
#include "Det_MemMap.h"
uint32 Det_numEventErrors[DET_NO_ECU_PARTS];               /**< @brief number of DET error ID*/
uint32 Det_numRuntimeEventErrors[DET_NO_ECU_PARTS];        /**< @brief number of runtime DET error ID*/
uint32 Det_numTransientEventErrors[DET_NO_ECU_PARTS];      /**< @brief number of transient DET error ID*/
...
uint16 Det_NextIdxList[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];
/**< @brief Head Index of Static Linked List */
uint16 Det_Head[DET_NO_ECU_PARTS];
/**< @brief Tail Index of Statuc Linked List */
uint16 Det_Tail[DET_NO_ECU_PARTS];

/**< @brief Next Index List of Det Trasient Error */
uint16 Det_Transient_NextIdxList[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];
/**< @brief Head Index of Static Linked List (Det Trasient Error) */
uint16 Det_Transient_Head[DET_NO_ECU_PARTS];
/**< @brief Tail Index of Static Linked List (Det Trasient Error) */
uint16 Det_Transient_Tail[DET_NO_ECU_PARTS];

/**< @brief Next Index of Det Runtime Error */
uint16 Det_Runtime_NextIdxList[DET_NO_ECU_PARTS][DET_MAX_NUMBER_OF_EVENTS];
/**< @brief Head Index of Static Linked (List Det Runtime Error) */
uint16 Det_Runtime_Head[DET_NO_ECU_PARTS];
/**< @brief Tail pointer of Static Linked List (Det Runtime Error) */
uint16 Det_Runtime_Tail[DET_NO_ECU_PARTS];
#define DET_STOP_SEC_VAR_CLEARED_16_NO_CACHEABLE
#include "Det_MemMap.h"
...
Det_LinkedListStateType Det_aErrorState[DET_NO_ECU_PARTS];
/**< @brief state of linked list Runtime Det error */
Det_LinkedListStateType Det_aRuntimeErrorState[DET_NO_ECU_PARTS];
/**< @brief state of linked list Transient Det error */
Det_LinkedListStateType Det_aTransientErrorState[DET_NO_ECU_PARTS];
...
Det_ModuleStateType Det_ModuleState[DET_NO_ECU_PARTS] = {DET_UNINIT};
...
boolean Det_OverflowErrorFlag[DET_NO_ECU_PARTS] = {FALSE};
/**< @brief Overflow flag of Runtime DET error */
boolean Det_OverflowRuntimeErrorFlag[DET_NO_ECU_PARTS] = {FALSE};
/**< @brief Overflow flag of Transient DET error */
boolean Det_OverflowTransientErrorFlag[DET_NO_ECU_PARTS] = {FALSE};
...
void Det_Init(void)
{
    uint32 u32PartId = (uint32)OsIf_GetUserId();

    /* Set to default value if out of range */
    if (u32PartId >= DET_NO_SUPPORTED_ECU_PARTS)
    {
        u32PartId = DET_NO_SUPPORTED_ECU_PARTS;
    }

    Det_InitLinkedList();

    Det_FreeNodesInLinkedList(&Det_aErrorState[u32PartId]);
    Det_FreeNodesInLinkedList(&Det_aRuntimeErrorState[u32PartId]);
    Det_FreeNodesInLinkedList(&Det_aTransientErrorState[u32PartId]);
}
                               ...
                               uint8 ErrorId)
{
    ...
    if (DET_UNINIT == Det_ModuleState[u32PartId])
    {
        Det_Init();
    }
    u16NodeToAdd = Det_InitDataNode(ModuleId,
                                    ...
                                    ApiId,
                                    ErrorId,
                                    &Det_aErrorState[u32PartId]);

    if (u16NodeToAdd != DET_NULL)
    {
        RetVal = Det_LinkNodeToHead(u16NodeToAdd, &Det_aErrorState[u32PartId]);
    }
    else
    {
                                      ...
                                      uint8 ErrorId)
{
    ...
    u16NodeToAdd = Det_InitDataNode(ModuleId,
                                    ...
                                    ApiId,
                                    ErrorId,
                                    &Det_aRuntimeErrorState[u32PartId]);

    if (u16NodeToAdd != DET_NULL)
    {
        RetVal = Det_LinkNodeToHead(u16NodeToAdd, &Det_aRuntimeErrorState[u32PartId]);
    }
    else
    {
        /* Can not add more node because of full linked list */
                                        ...
                                        uint8 FaultId)
{
    ...
    u16NodeToAdd = Det_InitDataNode(ModuleId,
                                    ...
                                    ApiId,
                                    FaultId,
                                    &Det_aTransientErrorState[u32PartId]);

    if (u16NodeToAdd != DET_NULL)
    {
        RetVal = Det_LinkNodeToHead(u16NodeToAdd, &Det_aTransientErrorState[u32PartId]);
    }
    else
    {
        /* Can not add more node because of full linked list */
...
LOCAL_INLINE void Det_InitLinkedList(void)
{
    ...
    Det_numEventErrors[u32PartId]          = 0U;
    Det_numRuntimeEventErrors[u32PartId]   = 0U;
    Det_numTransientEventErrors[u32PartId] = 0U;

    Det_OverflowErrorFlag[u32PartId]          = FALSE;
    Det_OverflowRuntimeErrorFlag[u32PartId]   = FALSE;
    Det_OverflowTransientErrorFlag[u32PartId] = FALSE;

    Det_aErrorState[u32PartId].pModuleId    = Det_ModuleId[u32PartId];
    Det_aErrorState[u32PartId].pInstanceId  = Det_InstanceId[u32PartId];
    Det_aErrorState[u32PartId].pApiId       = Det_ApiId[u32PartId];
    Det_aErrorState[u32PartId].pErrorId     = Det_ErrorId[u32PartId];
    Det_aErrorState[u32PartId].pNumErrors   = &Det_numEventErrors[u32PartId];
    Det_aErrorState[u32PartId].pNextIdxList = Det_NextIdxList[u32PartId];
    Det_aErrorState[u32PartId].pHead        = &Det_Head[u32PartId];
    Det_aErrorState[u32PartId].pTail        = &Det_Tail[u32PartId];
    Det_aErrorState[u32PartId].pOverflow    = &Det_OverflowErrorFlag[u32PartId];

    Det_aRuntimeErrorState[u32PartId].pModuleId    = Det_RuntimeModuleId[u32PartId];
    Det_aRuntimeErrorState[u32PartId].pInstanceId  = Det_RuntimeInstanceId[u32PartId];
    Det_aRuntimeErrorState[u32PartId].pApiId       = Det_RuntimeApiId[u32PartId];
    Det_aRuntimeErrorState[u32PartId].pErrorId     = Det_RuntimeErrorId[u32PartId];
    Det_aRuntimeErrorState[u32PartId].pNumErrors   = &Det_numRuntimeEventErrors[u32PartId];
    Det_aRuntimeErrorState[u32PartId].pNextIdxList = Det_Runtime_NextIdxList[u32PartId];
    Det_aRuntimeErrorState[u32PartId].pHead        = &Det_Runtime_Head[u32PartId];
    Det_aRuntimeErrorState[u32PartId].pTail        = &Det_Runtime_Tail[u32PartId];
    Det_aRuntimeErrorState[u32PartId].pOverflow    = &Det_OverflowRuntimeErrorFlag[u32PartId];

    Det_aTransientErrorState[u32PartId].pModuleId    = Det_TransientModuleId[u32PartId];
    Det_aTransientErrorState[u32PartId].pInstanceId  = Det_TransientInstanceId[u32PartId];
    Det_aTransientErrorState[u32PartId].pApiId       = Det_TransientApiId[u32PartId];
    Det_aTransientErrorState[u32PartId].pErrorId     = Det_TransientFaultId[u32PartId];
    Det_aTransientErrorState[u32PartId].pNumErrors   = &Det_numTransientEventErrors[u32PartId];
    Det_aTransientErrorState[u32PartId].pNextIdxList = Det_Transient_NextIdxList[u32PartId];
    Det_aTransientErrorState[u32PartId].pHead        = &Det_Transient_Head[u32PartId];
    Det_aTransientErrorState[u32PartId].pTail        = &Det_Transient_Tail[u32PartId];
    Det_aTransientErrorState[u32PartId].pOverflow    = &Det_OverflowTransientErrorFlag[u32PartId];

    Det_ModuleState[u32PartId] = DET_INIT;
}
#define DET_STOP_SEC_CODE
#include "Det_MemMap.h"
