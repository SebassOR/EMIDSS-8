...
#define LPSPI_IP_AR_RELEASE_MINOR_VERSION_C       9
#define LPSPI_IP_AR_RELEASE_REVISION_VERSION_C    0
#define LPSPI_IP_SW_MAJOR_VERSION_C               7
...
LOCAL_INLINE void Lpspi_Ip_HalfDuplexReceivePushCmdToFifo(uint8 Instance);
        ...
        else
        {
            ...
            if (LPSPI_IP_HALF_DUPLEX_RECEIVE == TransferType)
            {
...
#endif
                {
                    ...
                    Lpspi_Ip_HalfDuplexReceivePushCmdToFifo(Instance);
                }
            }
            else
            {
...
LOCAL_INLINE void Lpspi_Ip_HalfDuplexReceivePushCmdToFifo(uint8 Instance)
{
    ...
    if (0u != (State->HalfDuplexTcrCommand & LPSPI_TCR_CONT_MASK))
    {
        TcrCommand = State->HalfDuplexTcrCommand | LPSPI_TCR_CONTC_MASK;
    }
    else
    {
        TcrCommand = State->HalfDuplexTcrCommand;
    }

    if ((State->ExpectedFifoWrites > State->TxIndex) && (((Base->FSR) & LPSPI_FSR_TXCOUNT_MASK) == 0U))
    {
        /* first frame */
        if (State->TxIndex == 0U)
        {
            if (State->FirstCmd == TRUE)
            {
        ...
        /* last frame */
        else if (((State->ExpectedFifoWrites - 1U) == State->TxIndex) && (FALSE == State->KeepCs))
        {
...
static void Lpspi_Ip_TxDmaConfig(uint8 Instance)
{
    ...
    {
        if (FALSE == State->IsPreChanneHalfDuplexlRxMode)
        {
...
static void Lpspi_Ip_TxDmaContMemTransferHandler(const uint8 Instance, boolean* EndOfTransferFlag)
{
    LPSPI_Type *Base = Lpspi_Ip_apxBases[Instance];
    Lpspi_Ip_StateStructureType *State = Lpspi_Ip_apxStateStructureArray[Instance];
    ...
    else
    {
        ...
        if (LPSPI_IP_HALF_DUPLEX_TRANSMIT == State->ExternalDevice->DeviceParams->TransferType)
        {
            Lpspi_Ip_HalfDuplexPrepare(Instance, (uint8*)State->ContMemTransferCfg->TxBuffer[State->ContMemTransferCfg->CurrentChannelIndex]);
        }
#endif
...
static void Lpspi_Ip_IrqTxDmaEndTransferHandler(uint8 Instance, boolean* EndOfTransferFlag)
{
    ...
    {
        Lpspi_Ip_TxDmaContMemTransferHandler(Instance, EndOfTransferFlag);
    }
    else
#endif
#if (LPSPI_IP_ENABLE_DMA_REPEATED_TRANSFER_SUPPORT == STD_ON)
...
static void Lpspi_ConfigureCCR(uint8 Instance)
{
...
#endif
    {
        ...
        Base->CCR1 = State->ExternalDevice->Ccr1[State->ClockMode];
#else
        Base->CCR = State->ExternalDevice->Ccr;
        Base->CCR1 = State->ExternalDevice->Ccr1;
#endif
                                           ...
                                           Lpspi_Ip_CallbackType EndCallback)
{
    ...
    {
#if (LPSPI_IP_HALF_DUPLEX_MODE_SUPPORT == STD_ON)
        /* Clear bits related to a possible previous half duplex configuration */
        Base->CFGR1 &= ~(LPSPI_CFGR1_PCSCFG_MASK | LPSPI_CFGR1_OUTCFG_MASK | LPSPI_CFGR1_PINCFG_MASK);
#endif
        /* Mark the hardware as busy. */
        State->Status = LPSPI_IP_BUSY;
        ...
        if(TRUE == State->RepeatedTransfer)
        {
        	State->RepeatedTransferAttr = ExternalDevice->RepeatedTransferAttr;
                                                     ...
                                                     Lpspi_Ip_CallbackType EndCallback)
{
    ...
    uint8 Instance = 0u;
#if (LPSPI_IP_HALF_DUPLEX_MODE_SUPPORT == STD_ON)
    Lpspi_Ip_HalfDuplexType TransferType = SeqTxBufferList->TransferType;
#endif
    ...
    else
    {
        ...
        if(LPSPI_IP_FULL_DUPLEX != TransferType)
        {
            ...
            {
                Lpspi_Ip_HalfDuplexPrepare(Instance, RxBuffer);
            }
        }
        else
        {
        ...
        {
            /* Init TX channel */
            Lpspi_TransmitTxInit(Instance, (uint8*)State->ContMemTransferCfg->TxBuffer[State->ContMemTransferCfg->CurrentChannelIndex], SeqTxBufferList->TransferNumsByChannel[State->ContMemTransferCfg->CurrentChannelIndex]);
                                               ...
                                               Lpspi_Ip_CallbackType EndCallback)
{
    ...
    else
    {
        ...
        Base->CCR1 = FastTransferCfg[0u].ExternalDevice->Ccr1[State->ClockMode];
#else
        Base->CCR = FastTransferCfg[0u].ExternalDevice->Ccr;
        Base->CCR1 = FastTransferCfg[0u].ExternalDevice->Ccr1;
#endif
        Base->CR |= LPSPI_CR_MEN_MASK;
        SchM_Exit_Spi_SPI_EXCLUSIVE_AREA_15();
...
void Lpspi_Ip_IrqHandler(uint8 Instance)
{
    ...
    const Lpspi_Ip_StateStructureType* State = Lpspi_Ip_apxStateStructureArray[Instance];
    uint32 IrqFlags = 0u;
    uint32 IrqEnabled = 0u;

    IrqEnabled = Base->IER & (LPSPI_IER_TDIE_MASK | LPSPI_IER_RDIE_MASK | LPSPI_IER_TEIE_MASK | LPSPI_IER_REIE_MASK);
    ...
    else
    {
        ...
        /* clear all flags that were enabled */
        Base->SR &= ((LPSPI_IP_SR_W1C_MASK_U32 & (~(LPSPI_IER_TDIE_MASK | LPSPI_IER_RDIE_MASK | LPSPI_IER_TEIE_MASK | LPSPI_IER_REIE_MASK))) | IrqEnabled);
    }
}
