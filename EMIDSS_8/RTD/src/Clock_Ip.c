...
#define CLOCK_IP_SW_MAJOR_VERSION_C               7
...
static void Clock_Ip_CheckIrcoscClocks(Clock_Ip_ClockConfigType const * Config)
{
    ...
    if (Config->IrcoscsCount != 0U)
    {
        ...
        {
            for (Index = 0U; Index < (uint32)((uint32)Config->IrcoscsCount - 1U); Index++)
            {
...
static void Clock_Ip_CheckXoscClocks(Clock_Ip_ClockConfigType const * Config)
{
    ...
    if (Config->XoscsCount != 0U)
    {
        ...
        {
            for (Index = 0U; Index < (uint32)((uint32)Config->XoscsCount - 1U); Index++)
            {
...
static void Clock_Ip_CheckDividerTriggerClocks(Clock_Ip_ClockConfigType const * Config)
{
    ...
    if (Config->DividerTriggersCount != 0U)
    {
        ...
        {
            for (Index = 0U; Index < (uint32)((uint32)Config->DividerTriggersCount - 1U); Index++)
            {
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Clock_Ip_CheckClockConfiguration
 * Description   : Checks all clock names in the configuration
 *
 *END**************************************************************************/
static void Clock_Ip_CheckClockConfiguration(Clock_Ip_ClockConfigType const * Config)
{
...
void Clock_Ip_InitClock(Clock_Ip_ClockConfigType const * Config)
{
    ...
    DriverContext.HwDfsNo = CLOCK_IP_HARDWARE_DFS_ARRAY_SIZE;                        /* Number of fractional dividers */
