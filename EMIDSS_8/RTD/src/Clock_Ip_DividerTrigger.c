...
#define CLOCK_IP_DIVIDERTRIGGER_AR_RELEASE_MINOR_VERSION_C       9
#define CLOCK_IP_DIVIDERTRIGGER_AR_RELEASE_REVISION_VERSION_C    0
#define CLOCK_IP_DIVIDERTRIGGER_SW_MAJOR_VERSION_C               7
...
#ifdef ERR_IPV_MC_CGM_E050583
void Clock_Ip_SetFerd_TrustedCall(void);
void Clock_Ip_ResetFerd_TrustedCall(void);
#endif
#endif
...
static void Clock_Ip_ConfigureCgmXDivTrigCtrlTctlHhenUpdStat(Clock_Ip_DividerTriggerConfigType const* Config)
{
    ...
    if (NULL_PTR != Config)
    {
        ...
        {
#if (defined(ERR_IPV_MC_CGM_E050583))
    #ifdef CLOCK_IP_ENABLE_USER_MODE_SUPPORT
      #if (STD_ON == CLOCK_IP_ENABLE_USER_MODE_SUPPORT)
        OsIf_Trusted_Call(Clock_Ip_SetFerd_TrustedCall);
      #else
        Clock_Ip_SetFerd_TrustedCall();
      #endif
...
static void Clock_Ip_TriggerUpdateCgmXDivTrigCtrlTctlHhenUpdStat(Clock_Ip_DividerTriggerConfigType const* Config)
{
    ...
    #ifdef CLOCK_IP_ENABLE_USER_MODE_SUPPORT
      #if (STD_ON == CLOCK_IP_ENABLE_USER_MODE_SUPPORT)
        OsIf_Trusted_Call(Clock_Ip_ResetFerd_TrustedCall);
      #else
        Clock_Ip_ResetFerd_TrustedCall();
      #endif
    #endif /* CLOCK_IP_ENABLE_USER_MODE_SUPPORT */
#endif
        }
...
void Clock_Ip_SetFerd_TrustedCall(void)
{
    IP_MC_RGM->FERD |= (MC_RGM_FERD_D_FCCU_RST_MASK | MC_RGM_FERD_D_SWT0_RST_MASK | MC_RGM_FERD_D_SWT1_RST_MASK | MC_RGM_FERD_D_JTAG_RST_MASK | MC_RGM_FERD_D_DEBUG_FUNC_MASK);
}
void Clock_Ip_ResetFerd_TrustedCall(void)
{
    IP_MC_RGM->FERD &= ~(MC_RGM_FERD_D_FCCU_RST_MASK | MC_RGM_FERD_D_SWT0_RST_MASK | MC_RGM_FERD_D_SWT1_RST_MASK | MC_RGM_FERD_D_JTAG_RST_MASK | MC_RGM_FERD_D_DEBUG_FUNC_MASK);
