 ...
 *
 *  UPDATE: Jun 9, 2024
 *  Author: Noe Ortiz Terron
 *
 *  UPDATE: Jun 20 2026.
 *  DESCRIPTION: Migration for S32K312 Board. low level drivers API called changed not backwards compatibility
 *  with previous releases.
 *  AUTHOR: Gerardo Trejo Soria
 *  Version: 2.0v
 */
...
//#define I2C_TIMEOUT       (100000U) /* 100ms */
...
Std_ReturnType I2C0_u8WriteCmd(uint8_t u8SlaveAddr, uint8_t* u8Cmd)
{
    Lpi2c_Ip_StatusType status;
...
Std_ReturnType I2C1_u8WriteCmd(uint8_t u8SlaveAddr, uint8_t* u8Cmd, uint8_t len)
{
    Lpi2c_Ip_StatusType status;

    Lpi2c_Ip_MasterSetSlaveAddr(I2C_INSTANCE_1, u8SlaveAddr, false);
    status = Lpi2c_Ip_MasterSendDataBlocking(I2C_INSTANCE_1, u8Cmd, len, true, I2C_TIMEOUT);
...
Std_ReturnType I2C1_u8Write(uint8_t u8SlaveAddr, uint8_t *Buffer, uint8_t bytes)
{
    //uint8_t txData[2] = {u8Register, u8Data};
    Lpi2c_Ip_StatusType status;

    Lpi2c_Ip_MasterSetSlaveAddr(I2C_INSTANCE_1, u8SlaveAddr, false);
    status = Lpi2c_Ip_MasterSendDataBlocking(I2C_INSTANCE_1, Buffer, bytes, true, I2C_TIMEOUT);
